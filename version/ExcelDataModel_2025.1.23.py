...
class ExcelDataModel:
    ...
    # 合并文件夹内可选工作簿可选工作表可选区域下的数据以工作表形式储存到新工作簿
    def __mergeWorkbooks_to_workbook(self, path, area, filenames=[], sheets=[])-> opx.Workbook:
        new_wb = opx.Workbook()
        datadict = self.__getdata_for_workbooks(path, area, filenames, sheets)
        for filename, wb in datadict.items():
            wslist = [wb[sheetname] for sheetname in sheets] if sheets else wb.worksheets
            for ws_index in range(len(wslist)):
                ws = wslist[ws_index]
                new_ws = new_wb.create_sheet(f"{ws_index + 1}_{ws.title}")
    ...
    def mergeWorkbooks_saveto_newWorkbook(self, path, area={}, filenames=[], sheets=[])-> None:
        new_wb = self.__mergeWorkbooks_to_workbook(path, area, filenames, sheets)
